...
{
    my $config =
        { key => 'foo', location => 'St. Catharines, ON', ua => $mock };
    my $weather_conditions = Wunderground::Api::Conditions->new($config);
    is( $weather_conditions->location,
        "St. Catharines, ON",
        "the location is set"
    );
    is_deeply(
...
#Test that when a failed response is returned from the api that undef is returned
{
    ...
    is( $weather_conditions->current_conditions,
        undef, 'undef is returned when the api fails to respond' );
...
#Test that key is not set function dies
{
    ...
    dies_ok { $wc2->current_conditions; }
    'current_conditions dies when location is not set';
}

#Test that key is not set function dies
{
    my $wc3 = Wunderground::Api::Conditions->new( location => 'foo' );
    dies_ok { $wc3->current_conditions; }
    'current_conditions dies when key is not set';
}
